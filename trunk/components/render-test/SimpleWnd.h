...
namespace DuiEngine
{
	...
	class DUI_EXP CSimpleWndClass: public WNDCLASSEX
	{
	public:
		CSimpleWndClass(HINSTANCE hInst,LPCTSTR pszClassName,HBRUSH hBrush=(HBRUSH)(COLOR_WINDOW+1),UINT uStyle=CS_HREDRAW | CS_VREDRAW |CS_DBLCLKS, LPCTSTR pszCursor=IDC_ARROW)
		{
			...
			hCursor=LoadCursor(NULL,pszCursor);
			if(!hCursor) hCursor=LoadCursor(hInst,pszCursor);
		}
	};

	class DUI_EXP CSimpleWndHelper{
		friend class CSimpleWnd;
	public:
		static BOOL Init(CSimpleWndClass wndClass);
		static void Uninit();
	...
	private:
		CSimpleWndHelper(CSimpleWndClass wndClass);
		~CSimpleWndHelper();
