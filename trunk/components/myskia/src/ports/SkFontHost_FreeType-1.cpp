...
{
    SkTypeface* skface = SkTypeface::CreateFromName("SimSun11", s);
    return skface;
}
    ...
    if (perGlyphInfo & SkAdvancedTypefaceMetrics::kHAdvance_PerGlyphInfo) {
        if (FT_IS_FIXED_WIDTH(face)) {
            ...
            info->fGlyphWidths.reset(
                getAdvanceData<int16_t, FT_Face>(face,
                               face->num_glyphs,
                               glyphIDs,
                               glyphIDsCount,
                               getWidthAdvance));
...
//---------------------------------------------------------------------------
/*
bool FTFindProc(SkTypeface* face, SkTypeface::Style style, void* context)
{
    SkTypeface_FreeType* lface = static_cast<SkTypeface_FreeType*>(face);
    const SkTypefaceID_FT* lf = reinterpret_cast<const SkTypefaceID_FT*>(context);
    return (stricmp(lface->tfInfo.familyName, lf->familyName) == 0 && lface->tfInfo.style == lf->style);
}

SkTypeface* SkCreateTypefaceFromNameAndStyle(SkTypefaceID_FT& tfInfo) {
    SkTypeface* face = SkTypefaceCache::FindByProcAndRef(FTFindProc, &tfInfo);
    if (NULL == face) {
        face = SkTypeface_FreeType::Create(tfInfo);
        SkTypefaceCache::Add(face, tfInfo.style);
    }
    return face;
}
...
class SkFontStyleSetFT : public SkFontStyleSet {
public:
    SkFontStyleSetFT(const TCHAR familyName[]) {
        LOGFONT lf;
    ...
    virtual SkTypeface* createTypeface(int index) SK_OVERRIDE {
        SkTypefaceID_FT tfInfo;
        strcpy(tfInfo.familyName, "宋体");
        tfInfo.style = SkTypeface::Style::kNormal;

        return SkCreateTypefaceFromNameAndStyle(tfInfo);
...
class SkFontMgrFT : public SkFontMgr {
public:
...
protected:
    ...
    virtual void onGetFamilyName(int index, SkString* familyName) const SK_OVERRIDE {
        ...
        tchar_to_skstring("宋体", familyName);
        //tchar_to_skstring(fLogFontArray[index].elfLogFont.lfFaceName, familyName);
    }

    virtual SkFontStyleSet* onCreateStyleSet(int index) const SK_OVERRIDE {
        SkASSERT((unsigned)index < (unsigned)fLogFontArray.count());
        return SkNEW_ARGS(SkFontStyleSetFT, (fLogFontArray[index].elfLogFont.lfFaceName));
    }
    ...
    virtual SkFontStyleSet* onMatchFamily(const char familyName[]) const SK_OVERRIDE {
        ...
        return SkNEW_ARGS(SkFontStyleSetFT, (lf.lfFaceName));
    }
...
SkFontMgr* SkFontMgr_New_FT() {
    return SkNEW(SkFontMgrFT);
}

SkFontMgr* SkFontMgr::Factory()
{
    return SkFontMgr_New_FT();
}*/