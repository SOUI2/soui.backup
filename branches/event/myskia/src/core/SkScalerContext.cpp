...
SkScalerContext* SkScalerContext::allocNextContext() const {
    ...
    //return NULL;
    SkTypeface* newFace = SkFontHost::NextLogicalFont(fRec.fFontID, fTypeface->style());
    if (0 == newFace) {
        return NULL;
    }
    ...
    (SkScalerContext::Rec*)desc->addEntry(kRec_SkDescriptorTag,
                                          sizeof(fRec), &fRec);
    newRec->fFontID = newFace->uniqueID();
    desc->computeChecksum();

    return SkFontHost::CreateScalerContext(newFace, desc);
#endif
}
...
SkScalerContext* SkScalerContext::getNextContext() {
    ...
    if (NULL == next) {
        ...
        if (next->fTypeface.get() == fTypeface.get())
        {
            delete next;
            return NULL;
        }
