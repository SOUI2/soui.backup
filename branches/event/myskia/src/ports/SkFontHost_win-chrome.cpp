...
uint32_t SkFontHost::NextLogicalFont(uint32_t fontID) {
  ...
  // meaning here.
    ...
    SkTypeface* skface = SkTypeface::CreateFromName("»ªÎÄÐÐ¿¬", style);
    return skface->uniqueID();
}
...
void SkScalerContext_Windows::generateImage(const SkGlyph& glyph) {
    ...
    uint32_t total_size = GetGlyphOutlineW(fDDC, glyph.getGlyphID(fBaseGlyphCount), GGO_GRAY8_BITMAP | GGO_GLYPH_INDEX, &gm, 0, NULL, &fMat22);
    if (GDI_ERROR != total_size && total_size > 0) {
        uint8_t *pBuff = new uint8_t[total_size];
        if (NULL != pBuff) {
            total_size = GetGlyphOutlineW(fDDC, glyph.getGlyphID(fBaseGlyphCount), GGO_GRAY8_BITMAP | GGO_GLYPH_INDEX, &gm, total_size, pBuff, &fMat22);
