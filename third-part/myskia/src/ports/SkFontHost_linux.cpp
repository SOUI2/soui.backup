...
public:
    ...
    virtual SkTypeface_Custom* Clone(Style style) const
    {
        return newIns(style);
    }

    virtual SkTypeface_Custom* newIns(Style style) const
    {
        return NULL;
    }
...
public:
    ...
    SkTypeface_Custom* Clone(Style style) const
    {
        return newIns(style);
    }

    SkTypeface_Custom* newIns(Style style) const
    {
        SkString family;
        getFamilyName(&family);
        return new SkTypeface_File(style, false, fPath.c_str(), isFixedPitch(), family);
    }
...
public:
    ...
    virtual SkTypeface* matchStyle(const SkFontStyle& pattern) SK_OVERRIDE {
        ...
        // 如果是斜体，并且字体不支持，则创建一个
        /*if (!closest->isItalic() && pattern.isItalic())
        {
            SkTypeface_Custom* tf = NULL;
            
            if (closest->isBold())
            {
                tf = closest->Clone(SkTypeface::Style::kBoldItalic);
            }
            else
            {
                tf = closest->Clone(SkTypeface::Style::kItalic);
            }

            closest = tf;
            appendTypeface(closest);
        }*/

        SkString skname;
        closest->getFamilyName(&skname);

        SkDebugf("----+++++++++++++++++ mapping font [%s]\n", skname.c_str());
...
public:
    explicit SkFontMgr_Custom() {

        gDefaultFamily = NULL;
        const SkString filter("*.ttf,*.ttc");

        char buffer[512+16];
        GetWindowsDirectoryA(buffer,512);
        strcat(buffer,"\\Fonts\\");
        SkString baseDirectory(buffer);
        unsigned int count = 0;
        load_system_fonts(baseDirectory.c_str(), filter.c_str());
    }
...
protected:
                                               ...
                                               unsigned styleBits) const SK_OVERRIDE
    {
        ...
        if (NULL == tf && NULL != gDefaultFamily) {
...
private:
    ...
    void load_directory_fonts(const SkString& directory, const char* filter) {
        SkOSFile::Iter iter(directory.c_str(), filter);//".ttf");
        SkString name;
        ...
        while (iter.next(&name, false)) {
            ...
            //filename = SkOSPath::SkPathJoin(directory.c_str(), "simsun.ttc");
            ...
            //SkDebugf("------ load font <%s : %s> as a font\n", realname.c_str(), filename.c_str());
        ...
        while (dirIter.next(&name, true)) {
            ...
            SkString dirname(
                SkOSPath::SkPathJoin(directory.c_str(), name.c_str()));
            load_directory_fonts(dirname, filter);
        }
    }

    void load_system_fonts(const char* dir, const char* filter) {
        SkString baseDirectory(dir);
        SkTArray<SkString> out;
        SkStrSplit(filter, ",", &out);

        for (int i = 0; i < out.count(); ++i)
        {
            load_directory_fonts(baseDirectory, out[i].c_str());
        }
        ...
        static const char* gDefaultNames[] = {
            "SimSun", "Microsoft YaHei", "Arial", "Verdana", NULL
        };
