...
namespace DuiEngine
{
	...
	CSimpleWndHelper::CSimpleWndHelper(CSimpleWndClass wndClass)
		:m_sharePtr(NULL)
	{
		m_hInst=wndClass.hInstance;
		InitializeCriticalSection(&m_cs);
		m_hHeap=HeapCreate(HEAP_CREATE_ENABLE_EXECUTE,0,0);
		wndClass.lpfnWndProc=CSimpleWnd::StartWindowProc;
	...
	BOOL CSimpleWndHelper::Init(CSimpleWndClass wndClass)
	{
		if(s_Instance) return FALSE;
		s_Instance=new CSimpleWndHelper(wndClass);
		return s_Instance!=NULL;
	}

	void CSimpleWndHelper::Uninit()
	{
