...
namespace SOUI
{
	...
	void SWindow::_ClipChildRegion(SWindow *pWnd, IRegion *pRgn)
	{
		...
		while (pChild)
		{
			if (pChild->IsVisible())
			{
				if (pChild->m_uZorder > m_uZorder)
				{
					if (pChild->GetWindowRgn(pWndRgn))
					{
						pRgn->CombineRgn(pWndRgn, RGN_DIFF);
					}
					else
					{
						pRgn->CombineRect(pChild->GetWindowRect(), RGN_DIFF);
					}
				}
				else
				{//看整个分枝的zorder是不是在绘制范围内
					...
					if (pNextChild)
					{
						if (pNextChild->m_uZorder <= m_uZorder)
						{
							pChild = pNextChild;
							continue;
						}
					...
					{//最后一个节点时查看最后子窗口的zorder
						...
						if (pLastChild->m_uZorder < m_uZorder)
						{
							break;
						}
					}
	...
	void SWindow::GetShowRegion(IRegion *pRgn)
	{
		CAutoRefPtr<IRegion> pWndRgn;
		GETRENDERFACTORY->CreateRegion(&pWndRgn);
		SWindow *pChild = GetTopLevelParent()->GetWindow(GSW_FIRSTCHILD);
		while (pChild)
		{
